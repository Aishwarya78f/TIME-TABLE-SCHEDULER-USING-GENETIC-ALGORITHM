    ...
    def _select_instructors(self):
        ...
        for i in range(0, len(instructors)):
            returnInstructors.append(Instructor(instructors[i][0], instructors[i][1],
                                                self._select_instructor_availability(instructors[i][0])))
...
class Conflict:
    ...
    def str(self): return str(self._conflictType) + " " + str(" and ".join(map(str, self._conflictBetweenClasses)))
...
class DisplayMgr:
    ...
    def display_input_data():
        ...
        # DisplayMgr.display_schedule_instructors()
    ...
    def display_schedule_instructors(schedule):
        ...
        for i in range(0, len(instructors)):
            ...
            instructorsTable.add_row(
                [instructors[i].get_id(), instructors[i].get_name(), str(classSchedule), availability])
    ...
    def display_generation(population):
        ...
        for i in range(0, len(schedules)):
            table1.add_row(
                [str(i + 1), round(schedules[i].get_fitness(), 3), len(schedules[i].get_conflicts()), schedules[i].__str__()])
        print(table1)
...
displayMgr.display_schedule_as_table(population.get_schedules()[0])  # it will print fittest generation of schedule
...
while population.get_schedules()[0].get_fitness() != 1.0:
    ...
    displayMgr.display_schedule_instructors(population.get_schedules()[0])
print("\n\n")